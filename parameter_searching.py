...
import lgbm_for_param_2
import objective_function
import data_caller

X_train, X_test, y_val, y_val, X_test, y_test = data_caller.data_caller()
...
def simulated_annealing(X_train,
    ...
    columns = ['Iteration', 'Feature Count', 'Feature Set', 
               ...
               'Random Number', 'Outcome','Acc','Pre','Rec','F1','AUC', 'lr', 'num_boost_round','num_leaves','min_data_in_leaf','max_bin']
    ...
    lgbm_models, lr, num_boost_round,num_leaves,min_data_in_leaf,max_bin = lgbm_for_param_2.get_model(X_train, y_train)
    
    acc_test, acc_val,pre_val,rec_val,f1_val,auc_val, initial_full_feature_importance_dict = objective_function.get_acc(X_test, y_test, X_val, y_val, full_set, lgbm_models,feature_importance_df,column_index_mapping)
    ...
    lgbm_models, lr, num_boost_round,num_leaves,min_data_in_leaf,max_bin = lgbm_for_param_2.get_model(X_train_curr, y_train)
    ...
    acc_test, acc_val,pre_val,rec_val,f1_val,auc_val, initial_subset_feature_importance_dict = objective_function.get_acc(X_test_curr, y_test, X_val_curr, y_val, curr_subset, lgbm_models,second_feature_importance_df,column_index_mapping) # feature subset initialise한 curr_subset 넘겨줌
    ...
    results.loc[1, 'lr'] = lr
    results.loc[1, 'num_boost_round'] = num_boost_round
    results.loc[1, 'num_leaves'] = num_leaves
    results.loc[1, 'min_data_in_leaf'] = min_data_in_leaf
    results.loc[1, 'max_bin'] = max_bin
    results.loc[1, 'AUC'] = auc_val
    ...
    for i in range(maxiters):
        ...
        lgbm_models, lr, num_boost_round,num_leaves,min_data_in_leaf,max_bin = lgbm_for_param_2.get_model(X_train_new, y_train)
        ...
        second_feature_importance_df['feature'] = second_feature_list
        second_feature_importance_df.set_index('feature', inplace=True)
        metric, acc_val,pre_val,rec_val,f1_val,auc_val,initial_subset_feature_importance_dict = objective_function.get_acc(X_test_new, y_test, X_val_new, y_val, new_subset, lgbm_models,second_feature_importance_df,column_index_mapping)
        ...
        results.loc[i+1, 'lr'] = lr
        results.loc[i+1, 'num_boost_round'] = num_boost_round
        results.loc[i+1, 'num_leaves'] = num_leaves
        results.loc[i+1, 'min_data_in_leaf'] = min_data_in_leaf
        results.loc[i+1, 'max_bin'] = max_bin
        ...
        results.to_csv(f'sa_output_{dt_string}.csv', index=False)
...
if __name__ == '__main__':
    X_train, y_train, X_val, y_val, X_test, y_test = data_caller.data_caller()
