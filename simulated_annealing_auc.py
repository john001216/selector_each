...
def simulated_annealing(X_train,
    ...
    auc_test, acc_val,pre_val,rec_val,f1_val,auc_val, initial_full_feature_importance_dict = objective_function.get_auc(X_test, y_test, X_val, y_val, full_set, lgbm_models,feature_importance_df,column_index_mapping)
    ...
    auc_test, acc_val,pre_val,rec_val,f1_val,auc_val, initial_subset_feature_importance_dict = objective_function.get_auc(X_test_curr, y_test, X_val_curr, y_val, curr_subset, lgbm_models,second_feature_importance_df,column_index_mapping) # feature subset initialise한 curr_subset 넘겨줌
    # print(initial_subset_feature_importance_dict)
    
    prev_metric = auc_test
    ...
    for i in range(maxiters):
        ...
        second_feature_importance_df['feature'] = second_feature_list
        second_feature_importance_df.set_index('feature', inplace=True)
        metric, acc_val,pre_val,rec_val,f1_val,auc_val,initial_subset_feature_importance_dict = objective_function.get_auc(X_test_new, y_test, X_val_new, y_val, new_subset, lgbm_models,second_feature_importance_df,column_index_mapping)
        ...
        fig = px.line(melted_data, x='Epoch', y='Value', color='Metric',
                    ...
                    title='AUC Metrics Across Epochs on Test Set')

        # Update the chart in the placeholder
        progress_chart.plotly_chart(fig)
        
        progress_text_val.text(f'Best AUC in Test: {max(auc_val_list)}')
        progress_text_test.text(f'Best AUC in Val: {best_metric}')
