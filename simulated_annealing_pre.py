...
def simulated_annealing(X_train,
    ...
    acc_test, acc_val,pre_val,rec_val,f1_val,auc_val, initial_full_feature_importance_dict = objective_function.get_pre(X_test, y_test, X_val, y_val, full_set, lgbm_models,feature_importance_df,column_index_mapping)
    ...
    pre_test, acc_val,pre_val,rec_val,f1_val,auc_val, initial_subset_feature_importance_dict = objective_function.get_pre(X_test_curr, y_test, X_val_curr, y_val, curr_subset, lgbm_models,second_feature_importance_df,column_index_mapping) # feature subset initialise한 curr_subset 넘겨줌
    # print(initial_subset_feature_importance_dict)

    prev_metric = pre_test
    ...
    for i in range(maxiters):
        ...
        fig = px.line(melted_data, x='Epoch', y='Value', color='Metric',
                    ...
                    title='Precision Metrics Across Epochs on Test Set')

        # Update the chart in the placeholder
        progress_chart.plotly_chart(fig)
        
        progress_text_val.text(f'Best Precision in Test: {max(pre_val_list)}')
        progress_text_test.text(f'Best Precision in Val: {best_metric}')
