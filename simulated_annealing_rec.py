...
def simulated_annealing(X_train,
    ...
    acc_test, acc_val,pre_val,rec_val,f1_val,auc_val, initial_full_feature_importance_dict = objective_function.get_rec(X_test, y_test, X_val, y_val, full_set, lgbm_models,feature_importance_df,column_index_mapping)
    ...
    acc_test, acc_val,pre_val,rec_val,f1_val,auc_val, initial_subset_feature_importance_dict = objective_function.get_rec(X_test_curr, y_test, X_val_curr, y_val, curr_subset, lgbm_models,second_feature_importance_df,column_index_mapping) # feature subset initialise한 curr_subset 넘겨줌
    ...
    for i in range(maxiters):
        ...
        while True:
            ...
            print(new_subset)
            print(curr_subset)
            while None in curr_subset:
                curr_subset.discard(None)
            while None in new_subset:
                new_subset.discard(None)
            if move == 'Add':        
                ...
                if selected_key == None:
                    new_subset.add(random.choice(list(pending_cols)))
                    selected_key = random.choice(list(pending_cols))
                ...
                if selected_key_add == None:
                    new_subset.add(random.choice(list(pending_cols)))
                    selected_key_add = random.choice(list(pending_cols))
        ...
        second_feature_importance_df['feature'] = second_feature_list
        second_feature_importance_df.set_index('feature', inplace=True)
        metric, acc_val,pre_val,rec_val,f1_val,auc_val,initial_subset_feature_importance_dict = objective_function.get_rec(X_test_new, y_test, X_val_new, y_val, new_subset, lgbm_models,second_feature_importance_df,column_index_mapping)
